...
public class DrugController {
    ...
    @GetMapping
    public String getAddDrugPage(Model model) {
    ...
    @GetMapping("/all")
    public String getAllDrugsPage(Model model) {
        model.addAttribute("drugs", drugService.getAllDrugs());
        return "drug-table";
    }

    @GetMapping("/{drug-id}")
    public String getDrugInfoPage(@PathVariable("drug-id") UUID id, Model model) {
        model.addAttribute("categories", categoryService.getAllCategory());
        model.addAttribute("analogues", analogueClassService.getAllAnalogueClass());
